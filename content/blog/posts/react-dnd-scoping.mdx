...
If you haven't heard of Joggr (I'm the CTO & Co-Founder), we're the knowledge base built for devs and agents — connecting your codebase, conversations, and tools so both your engineers and your AI agents always have the context they need. (At the time of this post we were focused on docs in your IDE; the product has evolved since.)
...
export const DirectoryTreeView: React.FC<DirectoryTreeViewProps> = (props) => {
  ...
  const handleSidebarRef: React.RefCallback<HTMLSpanElement> = React.useCallback((node) => {
    setDndArea(node);
  }, []);

  const html5Options = React.useMemo(() => ({ rootElement: dndArea }), [dndArea]);
